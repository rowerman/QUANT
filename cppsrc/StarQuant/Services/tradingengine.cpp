...
int32_t tradingengine::run() {
    ...
    if (!g_except_stack.isDef()) {
        ...
        try {
            ...
            // set thread affinity
            // engine thread
            if (CConfig::instance()._msgq == MSGQ::KAFKA && CConfig::instance().kafka_strategy_bridge) {
                threads_.push_back(new std::thread(StrategyKafkaBridgeService));
            }
