...
// read server config
        ...
        try {
            ...
            if (config["msgq"]) {
                const string mq = config["msgq"].as<std::string>();
                if (mq == "kafka" || mq == "KAFKA") {
                    _msgq = MSGQ::KAFKA;
                } else if (mq == "nanomsg" || mq == "NANOMSG" || mq == "zmq" || mq == "ZMQ") {
                    _msgq = MSGQ::NANOMSG;
                } else {
                    _msgq = MSGQ::NANOMSG;
                }
            }
            if (config["brokerage_pair_url"]) {
                BROKERAGE_PAIR_PORT = config["brokerage_pair_url"].as<std::string>();
            }
            if (config["kafka_brokers"]) {
                kafka_brokers = config["kafka_brokers"].as<std::string>();
            }
            if (config["kafka_topic_server_pub"]) {
                kafka_topic_server_pub = config["kafka_topic_server_pub"].as<std::string>();
            }
            if (config["kafka_topic_engine_commands"]) {
                kafka_topic_engine_commands = config["kafka_topic_engine_commands"].as<std::string>();
            }
            if (config["kafka_topic_client_requests"]) {
                kafka_topic_client_requests = config["kafka_topic_client_requests"].as<std::string>();
            }
            if (config["kafka_topic_strategy_to_broker"]) {
                kafka_topic_strategy_to_broker = config["kafka_topic_strategy_to_broker"].as<std::string>();
            }
            if (config["kafka_topic_broker_to_strategy"]) {
                kafka_topic_broker_to_strategy = config["kafka_topic_broker_to_strategy"].as<std::string>();
            }
            if (config["kafka_group_relay"]) {
                kafka_group_relay = config["kafka_group_relay"].as<std::string>();
            }
            if (config["kafka_group_prefix"]) {
                kafka_group_prefix = config["kafka_group_prefix"].as<std::string>();
            }
            if (config["kafka_group_dataservice"]) {
                kafka_group_dataservice = config["kafka_group_dataservice"].as<std::string>();
            }
            if (config["kafka_group_strategy"]) {
                kafka_group_strategy = config["kafka_group_strategy"].as<std::string>();
            }
            if (config["kafka_strategy_bridge"]) {
                kafka_strategy_bridge = config["kafka_strategy_bridge"].as<bool>();
            }
#if !SQ_HAVE_RDKAFKA
            if (_msgq == MSGQ::KAFKA) {
                fmt::print(
                    "Warning: msgq=kafka requires librdkafka at build time; using NANOMSG.\n");
                _msgq = MSGQ::NANOMSG;
            }
#endif
