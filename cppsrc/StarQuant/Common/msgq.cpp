...
CMsgqEMessenger::CMsgqEMessenger(string name, string url_send) {
    ...
    if (msgq_recv_ == nullptr) {
        const string cgroup = CConfig::instance().kafka_group_prefix + "-" + name;
        msgq_recv_ = MakeMsgqSub(url_send, cgroup);
    }
}

CMsgqEMessenger::CMsgqEMessenger(string url_send) {
    if (msgq_recv_ == nullptr) {
        const string cgroup = CConfig::instance().kafka_group_prefix + "-default";
        msgq_recv_ = MakeMsgqSub(url_send, cgroup);
    }
}
...
CMsgqRMessenger::CMsgqRMessenger(string url_recv) {
    msgq_recv_ = MakeMsgqPull(url_recv, CConfig::instance().kafka_group_relay);
